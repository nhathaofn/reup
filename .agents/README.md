...
Promedia-specific skills are located in `.agents/skills/`. The root `AGENTS.md` file contains the accompanying instructions and must be distributed with the skill set.

## Use on Another Machine

Place both `AGENTS.md` and `.agents/skills/` in the Promedia Git checkout. Codex will automatically discover skills under `.agents/skills/` while working in the repository.

## Automatic Routing

Every skill keeps `policy.allow_implicit_invocation: true` in `agents/openai.yaml`. The agent must identify skills from the prompt's intent and boundaries based on the `description` in `SKILL.md`; the user does not need to invoke `$skill-name`. `feature-workflow` is the mandatory router for a non-trivial feature or behavior change and selects any additional specialized skills that are actually triggered. `infrastructure-boundary` protects the shared owner of FFmpeg, AI runtimes, storage, and queues; `lan-networking` preserves the LAN contract and evidence level; `external-runtime` keeps runtimes outside the client/server binary; `build-release` preserves artifact structure, size, and the Windows/Linux release gate.
