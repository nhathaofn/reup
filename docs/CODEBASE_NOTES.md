...
**Kiểm thử:** `npm run test:unit` là đường chạy offline, không gọi Gemini/rate API thật. `npm run test:smoke:srt` là live smoke opt-in, cần bốn biến `TEDIAPROS_GEMINI_SMOKE_KEY`, `TEDIAPROS_SRT_SMOKE_VIDEO`, `TEDIAPROS_SRT_SMOKE_SRT`, `TEDIAPROS_SRT_SMOKE_OUTPUT_DIR`, rồi phải xóa chúng. Tài liệu này không coi smoke là bằng chứng offline.
