...
Tạo một bản Windows portable từ codebase hiện tại để chạy song song với bản TediaPros cài từ GitHub mà không dùng chung dữ liệu, single-instance lock hoặc cơ chế tự cập nhật.
...
- Định danh bản local là `com.nhathaofn.tediapros.local` và `TediaPros Local`.
- Khi chạy portable packaged, dùng `PORTABLE_EXECUTABLE_DIR` để lưu `userData` vào thư mục `TediaPros Local Data` cạnh file `.exe`.
- Ưu tiên `TEDIAPROS_USER_DATA_DIR` hiện có để phục vụ smoke test hoặc override có chủ đích.
...
3. File đầu ra có dạng `dist-local/TediaPros-Local-<version>-portable.exe`.
4. Khi file portable khởi động, Main đổi `userData` sang `<thư mục exe>/TediaPros Local Data` trước `requestSingleInstanceLock()`.
5. Updater kiểm tra cờ local portable và trả trạng thái ổn định `none`, không gọi GitHub Releases.

## Kiểm tra chấp nhận

- Config chuẩn vẫn giữ `appId: com.nhathaofn.tediapros`, `productName: TediaPros`, output `dist` và publish GitHub.
...
Bản portable không tạo mục cài đặt/uninstall Windows. Khi di chuyển `.exe` sang máy/thư mục khác, nên di chuyển cùng thư mục `TediaPros Local Data` nếu muốn giữ API key, engine và thiết lập local.
