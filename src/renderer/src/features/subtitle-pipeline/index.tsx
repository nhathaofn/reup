...
function SubtitlePipelinePanel(): JSX.Element {
  const [outputDir, setOutputDir] = useTabOutputDir('tediapros.outputDir.subtitlePipeline')
  const [videoPath, setVideoPath] = useState('')
  const [sourceSrtPath, setSourceSrtPath] = useState('')
  const [runAsr, setRunAsr] = usePersistedState('tediapros.pipeline.asr', true)
  const [runOcr, setRunOcr] = usePersistedState('tediapros.pipeline.ocr', true)
  const [ocrMode, setOcrMode] = usePersistedState<'auto' | 'full'>('tediapros.pipeline.ocrMode', 'auto')
  const [aiEnabled, setAiEnabled] = usePersistedState('tediapros.pipeline.ai', true)
  const [keepIntermediates, setKeepIntermediates] = usePersistedState('tediapros.pipeline.raw', true)
  const [model, setModel] = usePersistedState('tediapros.pipeline.model', 'small')
  const [language, setLanguage] = usePersistedState('tediapros.pipeline.language', 'zh')
  const [quality, setQuality] = usePersistedState<'balanced' | 'accurate'>('tediapros.pipeline.quality', 'accurate')
  const [targetLocales, setTargetLocales] = usePersistedState<SrtLocaleTargetInput[]>('tediapros.pipeline.targetLocales', [])
  const [useGpu, setUseGpu] = usePersistedState('tediapros.pipeline.gpu', true)
